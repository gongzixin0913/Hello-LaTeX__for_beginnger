...
控制工程的学生，打交道最多的无非是 IEEE Transactions, Automatica, 或者国内的《自动化学报》。
...
同学，控制工程的核心是**鲁棒性（Robustness）**。LaTeX 也是如此，它不仅让你的论文排版具有鲁棒性（换个模板格式自动变，内容不用改），更能培养你结构化思考的习惯。

当你第一次看到编译出来的 PDF 里，那一个个精美的 $\int$ 和 $\Sigma$，以及那完美的对齐，你会发现，所有的折腾都是值得的。

加油，祝你的论文早日 Accept！如果有具体的报错，随时把 Error Log 发给我。